...
function main {
    ...
    if [ ! -d ${CONFLUENCE_INSTALL_DIR}/.install4j ] || [ ! -f ${CONFLUENCE_HOME}/confluence.cfg.xml ]; then
        ...
        cp -pr /usr/share/atlassian/confluence/driver/mysql-connector-java-5.1.44-bin.jar ${CONFLUENCE_INSTALL_DIR}/lib

        # Change ownership of the Java Mysql connector
        chown ${CONFLUENCE_USER}:${CONFLUENCE_GROUP} ${CONFLUENCE_INSTALL_DIR}/lib/mysql-connector-java-5.1.44-bin.jar
